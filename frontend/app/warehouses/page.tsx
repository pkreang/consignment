          ...
          <tbody>
            ...
            {data?.data.map((w) => (
              <tr key={w.warehouse_id} className="table-row">
                ...
                <td className="px-3 py-2"><span className={'pill ' + (w.active_flag ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-700')}>{w.active_flag ? 'Active' : 'Inactive'}</span></td>
            ...
            {data && data.data.length === 0 && <tr><td colSpan={4} className="px-3 py-4 text-slate-500">No warehouses.</td></tr>}
          </tbody>
        </table>
      </div>
      {data && (
