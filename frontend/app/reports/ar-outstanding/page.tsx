...
export default function ArOutstandingReport() {
  return (
    <ReportPage<Row>
      ...
      columns={[
        { label: 'Customer', cell: (r) => (<Link className="text-brand-600 dark:text-brand-400 hover:underline" href={`/ar?customerId=${r.customer_id}`}><div className="font-medium">{r.customer_name}</div><div className="font-mono text-xs text-surface-500">{r.customer_code}</div></Link>) },
